...
class DQAgent:
    ...
    def reset(self, g):
        ...
        self.adj_weighted = torch.tensor(self.graphs[self.games].W_weighted).unsqueeze(dim=0)
    ...
    def act(self, observation, dynamic):
        ...
        next_locs = torch.tensor(self.graphs[self.games].W[self.loc])
        ...
        if self.trip_len >= 20 / 3:  # each vehicle must return depot after nodes/cars num of jobs
            chosen_idx = 0
            self.trip_len = 0

        else:
            if self.epsilon_ > np.random.rand():
                chosen_idx = np.random.choice(np.where(next_locs == 1)[0])

            else:
                q_a = self.model(observation, self.adj)  # TODO self.static, self.dynamic
                q_a = q_a.detach().numpy()
                chosen_idx = np.where((q_a[0, :, 0] == np.max(q_a[0, :, 0][observation.numpy()[0, :, 0] == 0])))[0][0]
